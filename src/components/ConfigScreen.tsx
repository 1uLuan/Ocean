    ...
    <Show when={conf.configIsOpen}>
      <div class="absolute flex h-full w-full flex-col">
        <div class="flex h-full flex-row">
          <div class="w-autos h-full border-t border-b border-l border-(--border-secondary) bg-(--bg-primary) p-2">
            <For each={Object.entries(configButtons)}>
              {([name, [category, action]]) => (
                <button
                  class={`h-8 w-full rounded-md p-1 pr-6 text-left text-[0.9rem] text-(--text-primary) hover:bg-(--bg-hover-primary) ${
                    activeCategory() === category ? 'border border-(--border-primary)' : ''
                  }`}
                  onClick={action}
                >
          ...
          <div class="relative flex h-full w-full flex-col overflow-hidden border border-(--border-secondary) bg-(--bg-secondary) pt-3 pl-1 text-[0.9rem]">
            <div
            ...
            >
              Selecionar Tema
              <button
                class="h-6 w-full rounded-md pl-2 text-left hover:bg-(--bg-hover-primary)"
