...
export function BottomBar() {
  ...
  return (
    <div class="flex h-8 w-full flex-row items-center justify-center">
      ...
      <div class="relative h-full w-full m-2" onclick={() => setActive(!active())}>
        <div
        class="absolute -bottom-97.5 flex h-96 w-full flex-col overflow-scroll rounded-tl-md rounded-tr-md bg-(--bg-secondary) p-px transition-transform duration-150 ease-in-out"
          classList={{
            '-translate-y-105.5 border-t border-r border-l border-(--border-primary)': active(),
